...
namespace ScrubblerLib.Login
{
  ...
  public class User
  {
    ...
    public User(string username, string token, bool isSubscriber, IUserApi userApi)
    {
      ...
      UserAPI = userApi ?? throw new ArgumentNullException(nameof(userApi));
    }
    ...
    public async Task UpdateRecentScrobbles()
    {
      try
      {
        ...
        var page1 = await UserAPI.GetRecentScrobbles(Username, DateTimeOffset.UtcNow.Subtract(TimeSpan.FromHours(24)), null, false, 1, 1000);
        var page2 = await UserAPI.GetRecentScrobbles(Username, DateTimeOffset.UtcNow.Subtract(TimeSpan.FromHours(24)), null, false, 2, 1000);
        var page3 = await UserAPI.GetRecentScrobbles(Username, DateTimeOffset.UtcNow.Subtract(TimeSpan.FromHours(24)), null, false, 3, 1000);
